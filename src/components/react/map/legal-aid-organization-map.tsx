...
export default function LegalAidOrganizationMap() {
  const [position, setPosition] = useState<L.LatLng>(
    L.latLng(-6.2593186, 106.865371),
  );
  const [radius, setRadius] = useState<number>(1000);
  const [nearbyOBHs, setNearbyOBHs] = useState<OBH[]>([]); // Use the OBH type
  ...
  const fetchNearbyOBHs = useCallback(async (lat: number, lon: number, rad: number) => {
    ...
    try {
      ...
      const data = await response.json() as OBH[]; // Type assertion
      setNearbyOBHs(data);
    } catch (error) {
  ...
  return (
    <div className="relative z-0">
      {isLoading && (
        <div className="absolute top-1/2 left-1/2 z-[1000] transform -translate-x-1/2 -translate-y-1/2 bg-white/70 dark:bg-gray-800/70 p-2 rounded-lg backdrop-blur-sm">
          Loading...
        </div>
      )}
      <MapContainer
      ...
      >
        ...
        <RadiusWidget
          center={position}
          radius={radius}
          onRadiusChange={(newRadius) => handleRadiusChange(newRadius)}
          onCenterChange={(newLatLng) => handlePositionChange(newLatLng)}
        />

        {/* Render markers for nearby OBHs */}
        {nearbyOBHs.map((obh, index) => (
          ...
          >
            <Popup>
              <ShadcnCard className="w-[300px] border-0 shadow-none bg-transparent text-foreground">
                ...
                <CardContent className="pb-2 space-y-2">
                  ...
                  {/* You may want to add website field to OBH type if it exists */}
                  <div className="flex items-center gap-2">
                    <Globe className="h-4 w-4 text-muted-foreground" />
                    <a
                      href={`https://example.com`} // Replace with actual website if available
                      target="_blank"
      ...
      <div className="absolute top-4 right-4 z-[1000] rounded-lg bg-white/30 backdrop-blur-2xl p-2 shadow-md dark:bg-gray-300">
